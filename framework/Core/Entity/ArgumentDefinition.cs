...
{
    internal class ArgumentDefinition<T> : IArgumentDefinition
    {
        ...
        private object ChangeType(string value, Type conversionType)
        {
            ...
            if (conversionType == typeof(TimeSpan)) return TimeSpan.Parse(value);
        ...
        public void AddValue(string value)
        {
            ...
            try
            {
                if (value == null && IsBoolean) value = true.ToString();
                ...
                if (type.IsGenericType && type.GetGenericTypeDefinition().Equals(typeof(Nullable<>)))
                {
                    ...
                    else
                    {
                        if (underlyingType.IsEnum)
                            throw new ArgumentException("Missing enum value for nullable argument");
                    }
                }
                else convertedValue = ChangeType(value, type);

                AddRawValue(convertedValue);
            }
            catch (System.Exception ex)
            {
                throw new ArgumentException(string.Format("Unable to convert [{0}] into {1}", value, type.Name), ex);
            }
        }
        ...
        public void GetValues(object argumentClass)
        {
            ...
            if (_values.Count > 0)
            {
                PropertyInfo.SetValue(argumentClass,
                                       IsMultiValue
        ...
        {
            get { return PropertyInfo.GetCustomAttributes(typeof(ArgumentRequiredAttribute), true).Any(); }
        }
